...
- [x] **Task C7 — membros (organização).** Hook `use-members-table.ts`; `src/components/configuracoes/members/members-{table,columns}.tsx`. **Particular:** lista dentro de `SettingsLayout` — usa **`DataTable` base** (não `DataTableInfinite`, não fill-height). **"Finita" aqui ≠ "carrega tudo": a paginação por offset (`MembersPagination`, URL-state via `nuqs`) é mantida de propósito** — num Card de altura natural, sem container de altura fixa, o scroll infinito não tem onde ancorar, então offset é a escolha correta. Aplica `data-table-cells` + `SortableHeader`; mantém callbacks `onEditRoles`/`onRemove`. Sem `<PageHeader>` (o header é o de settings da Task A3).
